...
from .evaluate_results.address_confidence import get_address_confidence,get_every_address_confidence
from .evaluate_results.cp_confidence import get_cp_confidence,get_every_cp_confidence
from .evaluate_results.hq_confidence import get_hq_confidence,get_every_hq_confidence
from .evaluate_results.tp_confidence import get_tp_confidence,get_every_tp_confidence
from .evaluate_results.row_accuracy import moderate_confidence
...
def csv_exp_s_c(id_list,output_path):
    ...
    with open(dump_name, mode='w', encoding='utf8',
        ...
        attributes_a = ['title', 'link', 'description', 'comp_name',
                        ...
                        'revenue', 'funding', 'headquarters', 'No_of_employees', 'company_status'
                                                                                 ,'address_confidence',
                        'tp_confidence', 'headquarters_confidence', 'contact_confidence',
                 'Total_confidence']
        results_writer.writerow(attributes_a)
        ...
        for entry_id in id_list:
            ...
            data_list = []
            print(data_min[0].keys())
            for each_a in attributes_a:
                if(each_a in ['address_confidence','tp_confidence', 'headquarters_confidence', 'contact_confidence',
                 'Total_confidence']):
                    data_list.append(str(data_min[0][each_a]))
                else:
...
def simplified_update(id_list):
    ...
    csv_dump_col = refer_simplified_dump_col_min()
    # store data in a csv file
    # dump_name = 'C:\\Project_files\\armitage\\armitage_admin\\catalog\\temp_dumps\\' + str(
    #     id_list[0]) + '_company_dump_simplified.csv'
    # with open(dump_name, mode='w', encoding='utf8',newline='') as results_file:  # store search results in to a csv file
    #     results_writer = csv.writer(results_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
